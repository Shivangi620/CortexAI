  ...
  return (
    <>
      ...
      <div className="grid grid--stats">
        ...
        <StatCard 
          label="Data Mass" 
          value={datasetProfile?.size || "Unknown"} 
          meta="Estimated storage footprint" 
        />
        <StatCard 
          label="Imbalance" 
        ...
        <Panel title="🧹 Data Repair Assistant" subtitle="Preview or apply automated cleaning before training.">
          <div className="stack">
            ...
            <div className="inline-actions">
              <button className="button button--secondary" type="button" onClick={repairPreview}>Preview Cleaned Shape</button>
              <button className="button button--primary" type="button" onClick={repairApply}>Create Repaired Dataset</button>
            </div>
          ...
          {forms.repairPreviewData && (
            <div className="stack" style={{ marginTop: "1rem" }}>
              ...
              <DataTable 
                columns={Object.keys(forms.repairPreviewData.preview_records[0] || {}).map(k => ({ key: k, label: k }))}
      ...
      <div className="grid grid--one">
        <Panel title="🔍 Data Leakage & Quality Report" subtitle="Detect target leakage, constants, and future leakage.">
          <div className="stack">
            <div className="inline-actions">
              <button className="button button--secondary" type="button" onClick={runLeakageScan}>Run Leakage Scan</button>
            </div>
            ...
            {datasetLeakage && (
              <>
                ...
                <KeyValueList items={[
                  { label: "Duplicates", value: `${datasetLeakage.duplicate_rows} (${formatPercent(datasetLeakage.duplicate_pct)})` },
            ...
            <div className="stack">
              {datasetLineage.nodes.map((node, idx) => (
                <div key={node.id} className="feed-card">
                  <strong>{idx + 1}. {node.label}</strong>
                  <p className="tiny">{node.type || "Lineage event"} • {node.detail || "No detail available."}</p>
      ...
      <Panel title="🧬 Dataset Merge Studio" subtitle="Select datasets and join keys to create derived data.">
        <div className="stack">
          <div className="grid grid--two">
            <div className="field">
              ...
              <select value={forms.mergeLeftId} onChange={e => patchForm("mergeLeftId", e.target.value)}>
                ...
                {datasets.map(d => <option key={d.id} value={d.id}>{d.display_name} ({d.id.slice(0,8)})</option>)}
              </select>
            </div>
            <div className="field">
          ...
          <div className="grid grid--three">
            <div className="field">
              ...
              <select value={forms.mergeLeftKey} onChange={e => patchForm("mergeLeftKey", e.target.value)}>
                ...
                {leftColumns.map(k => <option key={k} value={k}>{k}</option>)}
              </select>
            </div>
            <div className="field">
              <span>Right Key</span>
              <select value={forms.mergeRightKey} onChange={e => patchForm("mergeRightKey", e.target.value)}>
                <option value="">Select key</option>
                {rightColumns.map(k => <option key={k} value={k}>{k}</option>)}
              </select>
            </div>
            <div className="field">
              <span>Join Type</span>
          ...
          <div className="inline-actions">
            <button className="button button--secondary" type="button" onClick={handleMergePreview}>Preview Join Stats</button>
            <button className="button button--primary" type="button" onClick={handleMergeApply}>Create Merged Dataset</button>
          </div>
